...
public class HanoiHintGame {
	 ...
	 * @return
	 */
	public static int smallestDisk(int peg) //returns the smallest disk on the peg given
	{
		int j=PegA.length+1;
		if(peg==1)
		{
	 ...
	 * @return
	 */
	public static int Find(int sizeofthedisk) //this is used to find the peg on which the required disk is
	{
		int peg=0;
		for(int i=0;i<PegA.length;i++)
		{
			if(PegA[i]==sizeofthedisk)
			{
				peg=1;
			}
		}
	 ...
	 * @return
	 */
	public static boolean makeMove(int disksToMove, int to)  
	{
		boolean flag=false;
		if(disksToMove==1 && smallestDisk(to)==disksToMove)
		{
			flag = false;
		}
		else if(disksToMove==1 && smallestDisk(to)!=disksToMove)
		{
			MoveDisk(Find(disksToMove),to);
			flag = true;
		}
		else if(disksToMove>=2 && to==Find(disksToMove))
		{
			flag = makeMove(disksToMove-1,to);
		}
		else if(disksToMove>=2 && to!=Find(disksToMove))
		{
			int x=Find(disksToMove);
			if(makeMove(disksToMove-1,6-to-x))
			{
				flag = true;
			}
			else if(!makeMove(disksToMove-1,6-to-x))
			{
				MoveDisk(x,to);
				flag = true;
			}
	...
	public static void main(String[] args) {
		...
		{
			System.out.println("There are no Disks on the pegs");
		}
		else
		{
			PegA = new int[n];
